...
function generarQR(documento) {
    ...
    // AQUÍ ES DONDE CUADRAS EL NOMBRE:
    // Si cambiaste tu usuario de GitHub a "PatrullerosHuila", el link sería así:
    const urlBase = "https://patrulleroscivicos.github.io/huila-carnet/"; 
    
    // Si compraste un dominio propio como "patrulleroscivicos.com", sería así:
    // const urlBase = "https://patrulleroscivicos.com/visor.html";
...
// ======================================================
// SINCRONIZAR CON LA NUBE (FIREBASE)
// ======================================================
function actualizarPatrulleroEnNube() {
    if (indexActual === null || !personal[indexActual]) {
        alert("Primero selecciona un patrullero.");
    ...
    // Guardamos en Firebase usando el documento (C.C.) como ID
    db.collection("patrulleros").doc(p.documento).set({
        nombre: p.nombre,
        documento: p.documento,
        rh: p.rh,
